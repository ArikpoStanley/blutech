...
const Table = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('http://3.88.1.181:8000/products/public/catalog?supplier=FragranceX&first=0&last=50&search=search');
      ...
      <h1 className='text-xl my-6 mx-5 font-bold'>Department List</h1>
        ...
        <table className='mb-5 w-full rounded-xl'>
          <tr className="w-full bg-[#F0F4FE] text-center text-gray-600 uppercase text-xs leading-normal mb-10 rounded-xl">
            <th className="py-3  items-center"> <input type='checkbox' className='mr-5'/>S/N</th>
        ...
        <tbody className=" text-xs text-left">
          {data.map((item, index) => (
            <tr key={index} className={`overflow-hidden transition-all duration-300 py-10 ${
              expandedRows.includes(index) ? 'max-h-none border-b border-gray-200 hover:bg-gray-100 ' : 'max-h-8 pr-10 border-b border-gray-200 hover:bg-gray-100  '
          }`} onClick={() => handleRowClick(index)}>
              <td className="py-7 px-3"><input type='checkbox' className='mr-5'/> {index + 1}</td>
              <td className=" px-4"><img src={item.Image_1} alt="Product" className='w-10 h-10'/></td>
              <td className=" px-4">{item.SKU}</td>
              <td className={`overflow-hidden transition-all duration-300 ${
                                    expandedRows.includes(index) ? 'max-h-none px-3 pr-5 w-[155px]' : 'max-h-12 px-2 pr-4 w-[155px]'
                                }`} >{item.Name}</td>
              <td className=" w-[130px] pr-2">{item.Title}</td>
              <td className=" max-w-[170px] px-6 "  ><p className={`overflow-hidden transition-all duration-300 ${
                                    expandedRows.includes(index) ? 'max-h-none' : 'max-h-12 pr-10'
